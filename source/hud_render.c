...
/*   Updated: 2023/03/10 14:06:02 by dbasting      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	hud_render(t_hud *hud, mlx_t *mlx)
{
	...
	if (id == -1)
		sl_error(SL_MEMFAIL);
	mlx_set_instance_depth(&hud->bg->instances[id], Z_BG0);
...
static void	bar_render(t_bar *bar, mlx_t *mlx)
{
	...
	if (id == -1)
		sl_error(SL_MEMFAIL);
	mlx_set_instance_depth(&bar->frame->instances[id], Z_BG2);
	id = mlx_image_to_window(mlx, bar->bar,
			bar->origin.x + BAR_MARGIN, bar->origin.y + BAR_MARGIN);
	if (id == -1)
		sl_error(SL_MEMFAIL);
	mlx_set_instance_depth(&bar->frame->instances[id], Z_BG1);
}
