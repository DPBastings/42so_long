...
/* t_point find_start(t_charmap *map)
 * Return the (topleftmost) point in `map` with the value CHR_STRT.
 * Return UINT_MAX, UINT_MAX if no such point is found.*/
static t_point	find_start(t_charmap *map)
{
	t_point	start;

	start.y = 1;
	while (start.y < map->dims.h - 1)
	{
		start.x = 1;
		while (start.x < map->dims.w - 1)
		{
			if (map->objs[start.y][start.x] == CHR_STRT)
				return (start);
			start.x++;
		}
		start.y++;
...
/* t_point *get_adjacent(t_point point, t_charmap *map)
...
static t_point	*get_adjacent(t_point point, t_charmap *map)
{
	t_point	*adjacent;
	size_t	i;
	...
	if ((size_t) point.x < map->dims.w - 1)
		set_point(adjacent + i++, point.x + 1, point.y);
	if ((size_t) point.y < map->dims.h - 1)
		set_point(adjacent + i++, point.x, point.y + 1);
	if (point.x > 0)
		set_point(adjacent + i++, point.x - 1, point.y);
	return (adjacent);
...
/* int is_passable(t_point point, t_charmap *map)
 * Checks whether `point` in `map` constitutes passable and unchecked terrain
 * (i.e. has a value other than CHR_WALL or CHR_CHECKED).*/
static int	is_passable(t_point point, t_charmap *map)
{
	return (map->objs[point.y][point.x] != CHR_WALL
			&& map->objs[point.y][point.x] != CHR_CHECKED);
}

static int	check_point(t_point point, t_charmap *map)
{
	t_point	*adjacent;
	size_t	i;
	int		res;

	if (map->objs[point.y][point.x] == CHR_EXIT)
		return (1);
